...
def api_add_device():
    ...
    try:
        ...
        # 准备 labels（如果有解析出的 labels）
        labels_to_save = parsed_labels if parsed_labels else []
        
        # 保存到 Git 存储
        success, msg, save_result = version_manager.save_device_version(
            device_id=new_device_id,
            new_labels=labels_to_save,
            username=username,
            change_summary=f'创建设备: {full_name}',
            protocol_meta={
                'name': full_name,
                'version': 'V1.0',
                'description': f'设备 {full_name}'
            }
        )
        
        if not success:
            return jsonify({'success': False, 'error': f'创建设备失败: {msg}'}), 400
