...
def create_version_record(old_labels, new_labels, version, username=None, change_summary=None):
    ...
    if not change_summary:
        parts = []
        if change_stats['added'] > 0:
            parts.append(f"新增 {change_stats['added']} 个 Label")
        if change_stats['deleted'] > 0:
            parts.append(f"删除 {change_stats['deleted']} 个 Label")
        if change_stats['modified'] > 0:
            parts.append(f"修改 {change_stats['modified']} 个 Label")
        auto_summary = '；'.join(parts) if parts else '无变更'
    
    return {
        'version': version,
        'updated_at': datetime.now().isoformat(),
        'updated_by': username or 'unknown',
        'username': username or 'unknown',  # 兼容字段
        'change_summary': change_summary or auto_summary,
