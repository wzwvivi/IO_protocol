...
def init_database():
    ...
    with get_db_connection() as conn:
        ...
        # 设备编辑锁表
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS device_edit_locks (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                device_id TEXT UNIQUE NOT NULL,
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_device_locks_device ON device_edit_locks(device_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_device_locks_user ON device_edit_locks(locked_by)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_repo_registry_ata ON repo_registry(ata_code)')
...
def db_create_protocol_version(db_device_id, version, name=None, source='user_created'):
    """创建协议版本
    
    Args:
        db_device_id: 设备的数据库主键 ID
        version: 版本号，如 'V1.0'
        name: 版本名称，如 '设备名称-V1.0'，默认使用 version
        source: 来源，如 'user_created', 'imported', 'directory'（目前未使用）
...
def db_get_version_history(device_id, limit=20):
    ...
    Returns:
        list: 版本历史记录列表，每条记录包含：
            - version: 版本号
            - updated_at: 更新时间
            - updated_by/username: 操作人
            - change_summary: 变更说明
            - change_stats: 变更统计 {added, modified, deleted}
            - diff_summary: 详细差异
            - label_snapshot: 旧版本快照
    ...
    with get_db_connection() as conn:
        ...
        for row in rows:
            ...
            # 从 diff_summary 计算 change_stats
            change_stats = {
                'added': len(diff_summary.get('added', [])),
                'modified': len(diff_summary.get('modified', [])),
                'deleted': len(diff_summary.get('removed', []))
            }
...
def db_acquire_device_lock(device_id, session_id, username, display_name, expires_at):
    """获取设备编辑锁
    ...
    with get_db_connection() as conn:
        ...
        # 先清理过期锁
        cursor.execute('''
            DELETE FROM device_edit_locks 
            WHERE expires_at < ? OR is_active = 0
        ''', (now,))
        
        # 检查是否已有锁
        cursor.execute('''
            SELECT * FROM device_edit_locks WHERE device_id = ?
        ''', (device_id,))
        ...
        if existing:
            ...
            if existing['locked_by'] == username:
                cursor.execute('''
                    UPDATE device_edit_locks 
                    SET session_id = ?, last_heartbeat_at = ?, expires_at = ?
                    WHERE device_id = ?
                ''', (session_id, now, expires_at, device_id))
                conn.commit()
                
                cursor.execute('SELECT * FROM device_edit_locks WHERE device_id = ?', (device_id,))
                updated = dict(cursor.fetchone())
                return True, '锁续租成功', updated
            else:
                # 被其他用户持有
                return False, f'设备正由 {existing["locked_by_display_name"] or existing["locked_by"]} 编辑中', existing
        
        # 创建新锁
        try:
            cursor.execute('''
                INSERT INTO device_edit_locks 
                (device_id, session_id, locked_by, locked_by_display_name, 
                 lock_acquired_at, last_heartbeat_at, expires_at, is_active)
                VALUES (?, ?, ?, ?, ?, ?, ?, 1)
            ''', (device_id, session_id, username, display_name, now, now, expires_at))
            conn.commit()
            
            cursor.execute('SELECT * FROM device_edit_locks WHERE device_id = ?', (device_id,))
            new_lock = dict(cursor.fetchone())
            return True, '获取锁成功', new_lock
        except sqlite3.IntegrityError:
            # 并发情况下可能已被其他人获取
            cursor.execute('SELECT * FROM device_edit_locks WHERE device_id = ?', (device_id,))
            existing = cursor.fetchone()
            if existing:
                existing = dict(existing)
                return False, f'设备正由 {existing["locked_by_display_name"] or existing["locked_by"]} 编辑中', existing
            return False, '获取锁失败', None


def db_release_device_lock(device_id, username, session_id=None):
    """释放设备编辑锁
    ...
    with get_db_connection() as conn:
        ...
        # 释放锁
        cursor.execute('DELETE FROM device_edit_locks WHERE device_id = ?', (device_id,))
        conn.commit()
        
        return True, '锁已释放'


def db_heartbeat_device_lock(device_id, username, session_id, expires_at):
    """心跳续租设备锁
    ...
    with get_db_connection() as conn:
        ...
        if existing['locked_by'] != username:
            return False, '锁已被他人持有', existing
        
        if existing['session_id'] != session_id:
            return False, '会话不匹配，锁可能已被接管', existing
        ...
        cursor.execute('SELECT * FROM device_edit_locks WHERE device_id = ?', (device_id,))
        updated = dict(cursor.fetchone())
        return True, '心跳成功', updated


def db_get_device_lock_status(device_id):
    """获取设备锁状态
...
def db_get_all_device_locks():
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute('SELECT * FROM device_edit_locks WHERE is_active = 1')
        rows = cursor.fetchall()
        
        return {row['device_id']: dict(row) for row in rows}


def db_get_devices_lock_status(device_ids):
    """批量获取设备锁状态
    ...
    with get_db_connection() as conn:
        ...
        for device_id in device_ids:
            cursor.execute('SELECT * FROM device_edit_locks WHERE device_id = ?', (device_id,))
            row = cursor.fetchone()
            result[device_id] = dict(row) if row else None
        
        return result


def db_force_release_device_lock(device_id):
    """强制释放设备锁（管理员操作）"""
    with get_db_connection() as conn:
        cursor = conn.cursor()
        
        cursor.execute('SELECT locked_by FROM device_edit_locks WHERE device_id = ?', (device_id,))
        row = cursor.fetchone()
        
        if not row:
            return True, '锁不存在'
        
        original_holder = row['locked_by']
        cursor.execute('DELETE FROM device_edit_locks WHERE device_id = ?', (device_id,))
        conn.commit()
...
def db_register_repo(ata_code, ata_name, repo_path):
    """注册 ATA 仓库"""
    now = datetime.now().isoformat()
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute('SELECT id FROM repo_registry WHERE ata_code = ?', (ata_code,))
        existing = cursor.fetchone()
        
        if existing:
            cursor.execute('''
                UPDATE repo_registry 
                SET ata_name = ?, repo_path = ?, updated_at = ?
                WHERE ata_code = ?
            ''', (ata_name, repo_path, now, ata_code))
        else:
            cursor.execute('''
                INSERT INTO repo_registry (ata_code, ata_name, repo_path, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?)
            ''', (ata_code, ata_name, repo_path, now, now))
        
        conn.commit()
        return True


def db_get_repo_info(ata_code):
    """获取 ATA 仓库信息"""
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM repo_registry WHERE ata_code = ?', (ata_code,))
        row = cursor.fetchone()
...
def db_list_repos():
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM repo_registry ORDER BY ata_code')
        return rows_to_list(cursor.fetchall())
