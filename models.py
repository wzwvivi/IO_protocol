...
def is_viewer():
    """检查当前用户是否有查看权限（所有登录用户）"""
    return is_logged_in()


def can_edit():
    """检查当前用户是否有编辑权限（operator 或 admin）"""
    return is_operator()


def operator_required(f):
    """设备人员权限验证装饰器（operator 或 admin）"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if 'user' not in session:
            if request.path.startswith('/api/'):
        ...
        if not is_operator():
            if request.path.startswith('/api/'):
                return jsonify({'error': '需要设备人员或管理员权限', 'code': 403}), 403
            return redirect(url_for('index'))
